...
module.exports = function build(dbPool) {
  ...
  router.get('/:recipeId', async (req, res, next) => {
    ...
    let [ingredients] = await dbPool.execute(/* sql */ `
    ...
    let [steps] = await dbPool.execute(/* sql */ `
    ...
    let [relatedProducts] = await dbPool.execute(/* sql */ `
      SELECT * FROM products
      LIMIT 4
    `);

    recipe.relatedProducts = relatedProducts;
  ...
  router.post('/:recipeId/review', async (req, res, next) => {
    console.log(req.body, req.params);
    await dbPool.execute(/* sql */ `
      INSERT INTO recipe_reviews (recipe_id, member_id, content, rating)
      VALUES (?, ?, ?, ?)
    `, [
      req.params.recipeId,
      req.body.memberId,
